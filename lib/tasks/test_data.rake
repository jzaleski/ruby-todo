namespace :test_data do
  task :create_list, [:user_id, :list_name] => :environment do |task, args|
    user_id, list_name = args[:user_id], args[:list_name]
  ...
  task :destroy_list, [:list_id_or_name] => :environment do |task, args|
    list_id_or_name = args[:list_id_or_name]
    abort(%{Must provide a "list_id" or "list_name"}) unless list_id_or_name
    list = if list_id_or_name =~ /\A[1-9][0-9]*\Z/
      List.find_by_id(list_id_or_name)
    else
      List.find_by_name(list_id_or_name)
    end
